...
# Télécharger les ressources nécessaires AVANT d’importer ResumeParser
...
MONGODB_URI = "mongodb+srv://alissamissaoui200:yoonminlover@cluster0.7hwo88y.mongodb.net/job-portal"
client = MongoClient(MONGODB_URI)
db = client["job-portal"]
collection = db["user_data"]  # "user_data" est comme une table
...
def run():
    ...
    # MongoDB Setup
MONGODB_URI = "mongodb+srv://alissamissaoui200:yoonminlover@cluster0.7hwo88y.mongodb.net/job-portal?retryWrites=true&w=majority&appName=Cluster0"
client = MongoClient(MONGODB_URI)

# Accès à la base de données et à la collection
db = client["job-portal"]
collection = db["user_data"]  # Équivaut à une table SQL

# Exemple : insertion d'un utilisateur (à utiliser quand tu as les données)
def insert_user_data(name, email, resume_score, timestamp, page_no,
    ...
    print(f"Données insérées avec l'ID : {result.inserted_id}")
